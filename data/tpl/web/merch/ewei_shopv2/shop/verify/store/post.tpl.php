...
<div class="page-header">
    当前位置：<span class="text-primary"><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>门店<?php  if(!empty($item['id'])) { ?>(<?php  echo $item['storename'];?>)<?php  } ?></span>
</div>
...
<div class="page-content">
    ...
    <form <?php if( mce('shop.verify.store' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9 col-xs-12">
                <?php if( mce('shop.verify.store' ,$item) ) { ?>
                <label class='radio-inline'><input type='radio' name='type' value='1' <?php  if($item['type']==1) { ?>checked<?php  } ?> /> 支持自提</label>
                <label class='radio-inline'><input type='radio' name='type' value='2' <?php  if($item['type']==2) { ?>checked<?php  } ?> /> 支持核销</label>
                <label class='radio-inline'><input type='radio' name='type' value='3' <?php  if($item['type']==3) { ?>checked<?php  } ?> /> 支持自提+核销</label>
                <?php  } else { ?>
                    <div class='form-control-static'><?php  if($item['type']==1) { ?>支持自提<?php  } else if($item['type']==2) { ?>支持核销{else if$item['type']==3}支持自提+核销<?php  } ?></div>
                <?php  } ?>
            </div>
        </div>
            ...
            <div class="form-group">
                ...
                <div class="col-sm-9 col-xs-12">
                    <?php if(mcv('sysset.notice.edit')) { ?>
                        <label class="checkbox-inline"><input type="checkbox" value="1" name='ordertype[]' <?php  if(in_array('1',$ordertype)) { ?>checked<?php  } ?> /> 下单打印</label>
                        <label class="checkbox-inline"><input type="checkbox" value="2" name='ordertype[]' <?php  if(in_array('2',$ordertype)) { ?>checked<?php  } ?> /> 付款打印</label>
                        <label class="checkbox-inline"><input type="checkbox" value="3" name='ordertype[]' <?php  if(in_array('3',$ordertype)) { ?>checked<?php  } ?> /> 确认收货打印</label>
