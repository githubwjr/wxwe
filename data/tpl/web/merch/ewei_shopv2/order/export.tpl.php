...
<div class="page-header"> 当前位置：<span class="text-primary">订单自定义导出</span></div>
<style type='text/css'>
...
<script language='javascript'>
    function resetFields() {
        tip.confirm('确认全部清空?', function () {
            $.get("<?php  echo merchUrl('order/export/reset')?>", function () {
                location.href = "<?php  echo merchUrl('order/export', array('dflag' => 1))?>";
            });
        });
    }
    ...
    function defaultFields() {
        tip.confirm('确认切换成默认模板?', function () {
            $.get("<?php  echo merchUrl('order/export/reset')?>", function () {
                location.href = "<?php  echo merchUrl('order/export')?>";
            });
        });
    }
        ...
        if (val == '') {
            ...
            $('#deltemplate').show().unbind('click').click(function () {
                if (confirm('确认要删除此导出模板?')) {
                    $.post("<?php  echo merchUrl('order/export/delete')?>", {tempname: val}, function (ret) {
                        ret = eval("(" + ret + ")");
                        if (ret.templates) {
                            $('#templates').empty();
            ...
            $.get("<?php  echo merchUrl('order/export/gettemplate')?>&tempname=" + currentTemplate, function (ret) {
                ret = eval("(" + ret + ")");
    ...
    function changedata(isnew) {
        ...
        $.post("<?php  echo merchUrl('order/export/save')?>", {columns: columns, tempname: currentTemplate}, function (ret) {
            if (isnew) {
                ret = eval("(" + ret + ")");
                if (ret.templates) {
                    $('#templates').empty();
