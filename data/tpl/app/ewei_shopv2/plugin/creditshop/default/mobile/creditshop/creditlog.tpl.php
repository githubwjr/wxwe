		...
		<div class='fui-content navbar'>
			<div class="fui-cell-group">
				<div class="fui-cell">
					...
					<div class="fui-cell-text"><p>我的<?php  echo $_W['shopset']['trade']['credittext'];?>: <?php  echo $credit;?></p></div>
					 <?php  if(!empty($this->set['crediturl'])) { ?>
					 	<a href="<?php  echo $this->set['crediturl']?>" class="external">
					 		<div class="fui-cell-remark"><?php  echo $_W['shopset']['trade']['credittext'];?>说明</div>
					 	</a>
					<?php  } ?>
		 	 	</div>
			 </div>
					...
					<%if item.type==0%>
						<a class="fui-list" href="<?php  echo mobileUrl('creditshop/log')?>">
							...
							<div class="fui-list-media" style="display:block;display:block;display:block;font-size:0.7rem;text-align: right;">
								<p class="text-danger2">-<%item.credit%><?php  echo $_W['shopset']['trade']['credittext'];?></p>
								<p>已兑换</p>
							</div>
						</a>
					<%else%>
						<a class="fui-list" href="<%if item.status>1%><?php  echo mobileUrl('creditshop/log')?><%else%>javascript:;<%/if%>">
							...
							<div class="fui-list-media" style="display:block;display:block;display:block;font-size:0.7rem;text-align: right;">
								<p class="text-danger2" style="background: none;">-<%item.credit%><?php  echo $_W['shopset']['trade']['credittext'];?></p>
								<%if item.status==1%>
