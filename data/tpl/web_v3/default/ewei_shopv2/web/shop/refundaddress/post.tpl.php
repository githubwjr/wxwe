...
<div class="page-header">
    当前位置：<span class="text-primary"><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>退货地址<?php  if(!empty($item['id'])) { ?>(<?php  echo $item['title'];?>)<?php  } ?></span>
</div>
...
<div class="page-content">
    ...
    <form <?php if( ce('shop.refundaddress' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data">
        ...
        <div class="form-group">
            <label class="col-lg control-label must"><?php  if(!empty($item['id'])) { ?>新<?php  } ?>地址</label>
            <div class="col-sm-9 col-xs-12">
                <?php if( ce('shop.refundaddress' ,$item) ) { ?>
                    <p>
