<?php defined('IN_IA') or exit('Access Denied');?><div class="page-header">
    当前位置：<span class="text-primary">积分兑换任务</span>
</div>
    ...
    <div class="page-toolbar m-b-sm m-t-sm" style="margin-bottom: 0">
        <div class="col-sm-4">
            <?php if(cv('exchange.score.setting')) { ?>
                <span class="">
                    <a class="btn btn-primary btn-sm" href="<?php  echo webUrl('exchange/score/setting',array('id'=>0));?>"><i class="fa fa-plus"></i> 添加积分兑换任务</a>
                </span>
            <?php  } ?>
        </div>
    ...
    <form action="" method="post">
        ...
        <tr>
            ...
            <tr>
                ...
                <td>
                    <a class="btn btn-default btn-sm btn-op btn-operation" title="查看" href="<?php  echo webUrl('exchange/score/dno',array('id'=>$value['id']));?>">
                         <span data-toggle="tooltip" data-placement="top" title="" data-original-title="查看兑换码">
                            <i class='icow icow-chakan-copy'></i>
                       </span>
                    </a>
                    <?php if(cv('exchange.score.setting')) { ?>
                    <a class="btn btn-default btn-sm btn-op btn-operation" href="<?php  echo webUrl('exchange/score/setting',array('id'=>$value['id']));?>" title="编辑">
                    ...
                    <?php  } ?>
                    <?php if(cv('exchange.score.delete')) { ?>
                    <a class="btn btn-default btn-sm btn-op btn-operation" data-toggle="ajaxRemove" href="<?php  echo webUrl('exchange/score/delete',array('id'=>$value['id']));?>" data-confirm="确认删除此兑换组？">
