...
<div class="page-content" style="display: block;">
    <div class="page-toolbar m-b-sm m-t-sm" style="margin-bottom: 0">
        <div class="col-sm-4">
            <?php if(cv('exchange.coupon.setting')) { ?>
                <span class="">
    ...
    <ul class="nav nav-arrow-next nav-tabs" id="myTab" >
        <li <?php  if($_W['action'] == 'coupon') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo webUrl('exchange/coupon')?>">进行中 (<span class="goods-ing"><?php  echo $allStart;?></span>)</a>
        </li>
        <li <?php  if($_W['action'] == 'coupon.nostart') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo webUrl('exchange/coupon/nostart')?>">未开始 (<span class="goods-ing"><?php  echo $allNostart;?></span>)</a>
        </li>
        <li <?php  if($_W['action'] == 'coupon.end') { ?>class="active"<?php  } ?>>
        <a href="<?php  echo webUrl('exchange/coupon/end')?>">已结束 (<span class="goods-sold"><?php  echo $allEnd;?></span>)</a>
        </li>
    </ul>
    ...
    <form action="" method="post">
        <div class="page-table-header" style="border: none;">
            <input type="checkbox">
            <div class="btn-group">
                <?php if(cv('exchange.coupon.delete')) { ?>
        ...
        <tr>
            ...
            <tr>
                ...
                <td colspan="2"><a href="<?php  echo webUrl('exchange/coupon/dno',array('id'=>$value['id']));?>"><?php  echo $value['title'];?></a></td>
                ...
                <td class="text-center">
                    <?php  if($_W['action']=='coupon') { ?>
                    <span class="label <?php  if($value['status']==1) { ?>label-primary<?php  } else { ?>label-danger<?php  } ?>" data-toggle="ajaxSwitch" data-confirm="确认暂停此兑换活动？" data-switch-refresh="true" data-switch-value="<?php  echo $value['status'];?>" data-switch-value0="0|已暂停|label label-default|<?php  echo webUrl('exchange/coupon/status',array('id'=>$value['id'],'status'=>0));?>" data-switch-value1="1|进行中|label label-success|<?php  echo webUrl('exchange/coupon/status',array('id'=>$value['id'],'status'=>1));?>"><?php  if($value['status']==1) { ?>进行中<?php  } else { ?>已暂停<?php  } ?></span>
                    <?php  } ?>
                    <?php  if($_W['action']=='coupon.nostart') { ?>
