...
<table class='table table-responsive' style='table-layout: fixed;'>
    ...
    <tr class='trorder'>
        <td colspan='4' >
            ...
            订单编号:  <?php  echo $item['ordersn'];?><?php  if($item['ispackage']) { ?><span class="text-danger">（套餐）</span><?php  } ?>
        ...
        <td colspan='4' style='text-align:right;font-size:12px;' class='aops'>
            ...
            <?php  if($item['merchid'] == 0) { ?>
                <?php  if(empty($item['statusvalue'])) { ?>
                    ...
                    <?php  if($item['ispackage'] ==0) { ?><a class='op'  data-toggle='ajaxModal' href="<?php  echo webUrl('order/op/changeprice',array('id'=>$item['id']))?>">
                         <i class="icow icow-gaijia" style="color: #999;display: inline-block;vertical-align: middle;margin-right: 3px"></i>订单改价</a><?php  } ?>
                    <?php  } ?>
                <?php  } ?>
            <?php  } ?>
        ...
        <td  rowspan="<?php  echo count($item['goods'])?>" style='text-align:center' >
            ￥<?php  echo number_format($item['price'],2)?> <a data-toggle='popover' data-html='true' data-trigger="hover" data-placement='right'
                ...
                <tr>
                    ...
                    <td  style='border:none;text-align:right;;'><span class='<?php  if(0<$item['changeprice']) { ?>success<?php  } else { ?>danger<?php  } ?>'><?php  if(0<$item['changeprice']) { ?>+<?php  } else { ?>-<?php  } ?>￥<?php  echo number_format(abs($item['changeprice']),2)?></span></td>
                </tr>
                <?php  } ?>
                <?php  if(intval($item['changedispatchprice'])!=0) { ?>
                <tr>
                    <td  style='border:none;text-align:right;'>卖家改运费：</td>
                    <td  style='border:none;text-align:right;;'><span class='<?php  if(0<$item['changedispatchprice']) { ?>success<?php  } else { ?>danger<?php  } ?>'><?php  if(0<$item['changedispatchprice']) { ?>+<?php  } else { ?>-<?php  } ?>￥<?php  echo abs($item['changedispatchprice'])?></span></td>
                </tr>
                <?php  } ?>
                <tr>
...
"
        ...
        <td   rowspan="<?php  echo count($item['goods'])?>" class='ops' style='line-height:20px;text-align:center' >
            <span class='text-<?php  echo $item['statuscss'];?>'><?php  echo $item['status'];?></span><br /><?php  if($item['merchid'] == 0) { ?><?php (!empty($this) && $this instanceof WeModuleSite || 1) ? (include $this->template('order/ops', TEMPLATE_INCLUDEPATH)) : (include template('order/ops', TEMPLATE_INCLUDEPATH));?><?php  } ?>
        </td>
