...
<div class="form-group">
    ...
    <div class="col-sm-9 col-xs-12" >
        ...
        <label class="radio-inline">
            <input type="radio" name="quickget" value="1" <?php  if($item['quickget'] == 1) { ?>checked="true"<?php  } ?> /> 可以
        </label>
        <label class="radio-inline">
            <input type="radio" name="quickget" value="0" <?php  if($item['quickget'] == 0) { ?>checked="true"<?php  } ?>/> 不可以
        </label>

        <span class='help-block'>会员是否可以通过链接直接获取优惠券.注意!!!此功能无法与支付与积分兑换优惠券功能同时使用!!!</span>

        <?php  } else { ?> <div class='form-control-static'>
        <?php  if($item['quickget']==1) { ?>可以<?php  } else { ?>不可以<?php  } ?>
    </div>
        <?php  } ?>
    </div>
</div>
