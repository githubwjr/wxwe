...
<div class="form-group cgt cgt-0" <?php  if($item['goodstype']!=0) { ?>style="display:none"<?php  } ?>>
    ...
    <div class="col-sm-9 col-xs-12">
        <?php if( ce('creditshop.goods' ,$item) ) { ?>
            <?php  echo tpl_selector('goodsid', array('required'=>false, 'preview'=>false,'url'=>webUrl('creditshop/goods/query'), 'items'=>$goods, 'readonly'=>true,'nokeywords'=>1,'autosearch'=>1, 'buttontext'=>'选择商品', 'placeholder'=>'请选择商品','callback'=>'select_goods'))?>
...
<div class="form-group cgt cgt-3" <?php  if($item['goodstype']!=3) { ?>style="display:none"<?php  } ?>>
    ...
    <div class="col-sm-9 col-xs-12" style='padding-left:0'>
        ...
        <div class="input-group fixmore-input-group">
            ...
            <input type="number" class="form-control" name="maxpacketmoney" <?php  if(!empty($item)) { ?> disabled<?php  } ?> value="<?php  echo $item['maxpacketmoney'];?>">
            <span class="input-group-addon">元</span>
        </div>
        <?php  } else { ?>
