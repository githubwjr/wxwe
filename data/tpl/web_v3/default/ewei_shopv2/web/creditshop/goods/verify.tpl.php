...
<div class="form-group verify_groups" <?php  if(empty($item['isverify'])) { ?>style="display:none;"<?php  } ?>>
    ...
    <div class="col-sm-9 col-xs-12">
        ...
        <label class="radio-inline">
            <input type="radio" name='isendtime' value="0" <?php  if(empty($item['isendtime'])) { ?>checked<?php  } ?> /> 指定时间核销
        </label>
        <label class="radio-inline">
            <input type="radio" name='isendtime' value="1" <?php  if($item['isendtime']==1) { ?>checked<?php  } ?> /> 限时核销
        </label>
        <?php  } else { ?>
        <div class='form-control-static'><?php  if(empty($item['isendtime'])) { ?>指定时间核销<?php  } else { ?>限时核销<?php  } ?></div>
        <?php  } ?>
    </div>
</div>
