...
<div class="page-heading"> 
    <span class='pull-right'>
               <?php if(cv('universalform.temp.add')) { ?>
    ...
    <div class="page-toolbar row m-b-sm m-t-sm">
        <div class="col-sm-4">
            ...
            <div class="input-group-btn">
                ...
                <?php if(cv('universalform.temp.delete')) { ?>
...
<table class="table table-responsive">
    ...
    <tr>
        ...
        <td> 
            <?php if(cv('universalform.temp.edit|universalform.temp.view')) { ?><a class='btn btn-default' href="<?php  echo webUrl('universalform/temp/edit', array( 'id' => $item['id']))?>"><i class='fa fa-edit'></i> <?php if(cv('universalform.temp.edit')) { ?>编辑<?php  } else { ?>查看<?php  } ?></a><?php  } ?>
            <?php if(cv('universalform.data')) { ?><a class='btn btn-default' href="<?php  echo webUrl('universalform/data', array('typeid' => $item['id']))?>" title='查看已有数据'><i class='fa fa-list'></i>查看数据</a><?php  } ?>
            <?php if(cv('universalform.temp.delete')) { ?>
            <a data-toggle='ajaxRemove' class='btn btn-default btn-sm'  href="<?php  echo webUrl('universalform/temp/delete', array('id' => $item['id']))?>" data-confirm="确认删除此模板吗？"><i class='fa fa-remove'></i> 删除</a><?php  } ?>
