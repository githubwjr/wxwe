...
<div class="page-header">
    ...
    当前位置：<span class="text-primary">未开始</span>
</div>
<div class="page-content">
    <form action="" method="post" class="form-horizontal form-search" role="form">
        ...
        <div class="input-group pull-right col-md-6">
            <input type="text" class="input-sm form-control" name="search"  value="<?php  echo $_GPC['search'];?>" placeholder="商品名称" style="width: 300px;float: right;"> <span class="input-group-btn">
    ...
    <table class="table table-responsive">
        ...
        <tr>
            ...
            <td><a title="<?php  $day = intval((strtotime($value['start_time'])-time())/3600/24);$hour = intval((strtotime($value['start_time'])-time()-$day*3600*24)/3600);?>距离活动结束剩余:<?php  if($day > 0) { ?><?php  echo $day;?>天<?php  } else { ?><?php  echo $hour;?>小时<?php  } ?>"><font style="color: red"><?php  if($day > 0) { ?><?php  echo $day;?>天<?php  } else { ?><?php  echo $hour;?>小时<?php  } ?></font></a></td>

            <td  style="overflow:visible;position:relative">
                <?php if(cv('bargain.react')) { ?>
                <a href="<?php  echo webUrl('bargain/startnow',array('id'=>$value['id']));?>" class='btn btn-default btn-sm btn-op btn-operation' data-toggle="ajaxRemove">
                     <span data-toggle="tooltip" data-placement="top" title="" data-original-title="开始">
