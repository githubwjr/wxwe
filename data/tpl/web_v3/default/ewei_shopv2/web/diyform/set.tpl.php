...
<?php  } ?>
    ...
    <?php  if(p('dividend')) { ?>
    <div class="form-group">
        <label class="col-lg control-label">团队分红申请资料</label>


        <div class="col-sm-9 col-xs-12">
            <?php if(cv('diyform.set.edit')) { ?>
            <label class="radio radio-inline" style="float: left;">
                <input type="radio" value="1" name="setdata[dividend_diyform_open]" <?php  if($set['dividend_diyform_open'] == 1) { ?>checked<?php  } ?>> 启用
            </label>
            <label class="radio radio-inline" style="float: left;">
                <input type="radio" value="0" name="setdata[dividend_diyform_open]" <?php  if(empty($set['dividend_diyform_open'])) { ?>checked<?php  } ?>> 关闭
            </label>
            <select id="dividend_diyform" name="setdata[dividend_diyform]" class="form-control" style="width:167px; float: left;margin-left:10px;">
                <option value="0" <?php  if($set['dividend_diyform']==0) { ?>selected<?php  } ?>>--选择自定义表单--</option>
                <?php  if(is_array($form_list)) { foreach($form_list as $key => $value) { ?>
                <option value="<?php  echo $value['id'];?>" <?php  if($set['dividend_diyform']==$value['id']) { ?>selected<?php  } ?>><?php  echo $value['title'];?></option>
                <?php  } } ?>
            </select>

            <?php  } else { ?>
            <div class="form-control-static">
                <?php  if($set['dividend_diyform_open'] == 1) { ?>
                开启 -   <?php  if(is_array($form_list)) { foreach($form_list as $key => $value) { ?><?php  if($set['dividend_diyform']==$value['id']) { ?><?php  echo $value['title'];?><?php  } ?><?php  } } ?>
                <?php  } else { ?>
                关闭
                <?php  } ?>
            </div>
    ...
    <?php  if(p('groups')) { ?>
    <div class="form-group">
        <label class="col-lg control-label">拼团下单</label>
        <div class="col-sm-9 col-xs-12">
            <?php if(cv('diyform.set.edit')) { ?>
            <label class="radio radio-inline" style="float: left;">
                <input type="radio" value="1" name="setdata[groups_diyform_open]" <?php  if($set['groups_diyform_open'] == 1) { ?>checked<?php  } ?>> 启用
            </label>
            <label class="radio radio-inline" style="float: left;">
                <input type="radio" value="0" name="setdata[groups_diyform_open]" <?php  if(empty($set['groups_diyform_open'])) { ?>checked<?php  } ?>> 关闭
            </label>
            <select id="groups_diyform" name="setdata[groups_diyform]" class="form-control" style="width:167px; float: left;margin-left:10px;">
                <option value="0" <?php  if($set['groups_diyform']==0) { ?>selected<?php  } ?>>--选择自定义表单--</option>
                <?php  if(is_array($form_list)) { foreach($form_list as $key => $value) { ?>
                <option value="<?php  echo $value['id'];?>" <?php  if($set['groups_diyform']==$value['id']) { ?>selected<?php  } ?>><?php  echo $value['title'];?></option>
                <?php  } } ?>
            </select>
