...
<div class="page-header">
    当前位置：<span class="text-primary"><?php  if(!empty($item['id'])) { ?>编辑<?php  } else { ?>添加<?php  } ?>版块分类 <small><?php  if(!empty($item['id'])) { ?>修改【<?php  echo $item['name'];?>】<?php  } ?></small></span>
</div>
<div class="page-content">
    ...
    <form  <?php if( ce('sns.category' ,$item) ) { ?>action="" method="post"<?php  } ?> class="form-horizontal form-validate" enctype="multipart/form-data" >
                <?php  if(!empty($item)) { ?>
                <div class="form-group">
                    <label class="col-lg control-label">分类链接(点击复制)</label>
                    <div class="col-sm-9 col-xs-12">
                        <p class='form-control-static'>
                            <a href='javascript:;' title='点击复制链接' class="js-clip" data-url="<?php  echo mobileUrl('sns/board/lists',array('cid'=>$item['id']),true)?>">
                                <?php  echo mobileUrl('sns/board/lists',array('cid'=>$item['id']),true)?>
                            </a>
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-9 col-xs-12">
                        ...
                        <?php  } ?>
                       <input type="button" name="back" onclick='history.back()' <?php if(cv('sns.category.add|sns.category.edit')) { ?>style='margin-left:10px;'<?php  } ?> value="返回列表" class="btn btn-default" />
                    </div>
                </div>
