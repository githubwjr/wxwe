...
//        var_dump($info);die;
	...
	public function checkMemberSNS($sns) 
	{
		...
		if( $sns == "wx" ) 
		{
			...
			$appid = "wx32c2647c13dd0c38";
			$secret = "a9a9a8234887162ab3020428542b06d6";
			...
			if( empty($userinfo["openid"]) ) 
			{
          	...
          	$_GPC['openid']=$userinfo["unionid"];
          	$_GPC['access_token']=$userinfo["access_token"];
          	$_GET['openid']=$userinfo["unionid"];
          	$_GET['access_token']=$userinfo["access_token"];
			$userinfo["openid"] = "sns_wx_" . $userinfo["unionid"];
		}
		else 
		{
			if( $sns == "qq" ) 
