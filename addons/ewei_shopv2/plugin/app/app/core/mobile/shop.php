...
class Shop_EweiShopV2Page extends AppMobilePage
{
	public function get_shopindex()
	{
		...
		$customer= $_W['shopset']['app']['customer'];
		$newsorts = array();
		...
		app_json(array('uniacid' => $uniacid, 'sorts' => array_values($newsorts), 'goodsstyle' => $goodsstyle, 'customer' => $customer, 'copyright' => !empty($copyright) && !empty($copyright['copyright']) ? $copyright['copyright'] : ''));
	}
   ...
   public function get_danmus()
    ...
    $danmutime=$danmu['data'][$index]['time'];
	 $hideqdgg = $_W['shopset']['app']['hideqdgg'];
     $hidedddm = $_W['shopset']['app']['hidedddm'];
    $badvs = pdo_fetchall('select id,badvname,link,thumb from ' . tablename('ewei_shop_badv') . ' where enabled=1 order by displayorder desc', array(':uniacid' => $uniacid));
	app_json(array('danmu' => $danmu['data'] ,'danmutime' => $danmutime,'badvs' => $badvs,'hideqdgg' => $hideqdgg,'hidedddm' => $hidedddm));
  }
