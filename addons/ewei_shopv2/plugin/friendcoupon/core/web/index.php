...
class Index_EweiShopV2Page extends PluginWebPage
{
    ...
    public function activity_list()
    {
        ...
        $list = pdo_fetchall("select * from " . tablename('ewei_shop_friendcoupon') . " where 1 {$condition} and uniacid = :uniacid and deleted = :deleted order by create_time desc limit " . ($pindex - 1) * $psize . "," . $psize, array(
            ":uniacid" => $_W['uniacid'],
            ":deleted" => 0
        ));
        ...
        foreach ($list as &$item) {
            ...
            if ($time < $item['activity_start_time']) {
                $item['status'] = 0; 
            } elseif ($time > $item['activity_end_time'] || (!empty($item['stop_time']) && $time > $item['stop_time'])) {
                $item['status'] = -1; 
            } else {
                $item['status'] = 1;
            }
    ...
    public function post()
    {
        ...
        if ($_W['ispost']) {
            $data = array(
                ...
                'people_count'        => (int)$_GPC['people_count'], 
                'coupon_money'        => (float)$_GPC['coupon_money'], 
                'duration'            => (int)$_GPC['duration'], 
                'allocate'            => (int)$_GPC['allocate'], 
                'upper_limit'         => (float)$_GPC['upper_limit'], 
                'launches_limit'      => (int)$_GPC['launches_limit'], 
                'activity_start_time' => strtotime($_GPC['activity_time']['start']), 
                'activity_end_time'   => strtotime($_GPC['activity_time']['end']), 
                'desc'                => $_GPC['desc'], 
                'use_condition'       => (float)$_GPC['use_condition'], 
                'use_time_limit'      => (int)$_GPC['use_time_limit'], 
                'use_start_time'      => strtotime($_GPC['use_time']['start']), 
                'use_end_time'        => strtotime($_GPC['use_time']['end']), 
                'limitdiscounttype'   => (int)$_GPC['limitdiscounttype'], 
                'limitgoodcatetype'   => (int)$_GPC['limitgoodcatetype'], 
                'limitgoodtype'       => (int)$_GPC['limitgoodtype'], 
                'use_valid_days'      => (int)$_GPC['use_valid_days'], 
            );
            ...
            if( round($data['upper_limit'], 2) > $minLimit) {
            ...
            $titleExists = pdo_fetchcolumn("select count(1) from " . tablename('ewei_shop_friendcoupon') . " where uniacid = :uniacid and deleted = :deleted and title = :title and id <> :id", array(
                ':uniacid' => $_W['uniacid'],
                ':title'   => $data['title'],
                ':deleted' => 0,
                ':id'      => $id
