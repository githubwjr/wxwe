...
class Goods_EweiShopV2Page extends PluginMobileLoginPage
{
	...
	public function goodsCheck()
	{
		...
		try {
			...
			if ($goods['more_spec'] == 1) {
				...
				// $specArr = pdo_getall('ewei_shop_goods_spec', array('goodsid' => $group_goods['gid'], 'uniacid' => $_W['uniacid']), array('id', 'title'), '', array('displayorder' => 'desc'));
				$specArr = pdo_getall('ewei_shop_goods_spec', array('goodsid' => $group_goods['gid']), array('id', 'title'), array('displayorder' => 'desc'));
				// $specArr = pdo_getall('ewei_shop_goods_spec');
	...
	public function get_option()
	{
		...
		if (!empty($specArr)) {
			...
			$goods_option = pdo_get('ewei_shop_groups_goods_option', array('specs' => $spec_id, 'uniacid' => $_W['uniacid'],'groups_goods_id'=>$id));
			// echo  $spec_id,$_W['uniacid'];die;
			// print_r($goods_option);die;
