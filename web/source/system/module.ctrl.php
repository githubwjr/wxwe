...
if ($do == 'save_module_info') {
	...
	if (!empty($module_info['logo']) && $type == 'logo') {
		$image = parse_path($module_info['logo']);if(preg_match("/(jpg|gif|png|bmp)$/", $image)) {$image = file_get_contents($image);}
		$result = file_put_contents(IA_ROOT . "/addons/" . $module_info['name'] . '/icon-custom.jpg', $image);
	}
	if (!empty($module_info['preview']) && $type == 'preview') {
		$image = parse_path($module_info['preview']);if(preg_match("/(jpg|gif|png|bmp)$/", $image)) {$image = file_get_contents($image);}
